...
public class InfinispanConnectionImpl extends BasicConnection implements InfinispanHotRodConnection { 
	 ...
	 * {@inheritDoc}
	 *
	 * @see org.teiid.translator.infinispan.hotrod.InfinispanHotRodConnection#getQueryBuilder(java.lang.String)
	 */
	@Override
	public QueryBuilder getQueryBuilder(String tableName) throws TranslatorException {
		
		Class<?> type = getCacheClassType();
	...
	public Class<?> getCacheClassType() throws TranslatorException {		
		...
		Class<?> type = getConfig().getCacheClassType();
		if (type != null) {
			return type;
		}
	...
	public String getPkField() {
		return (config == null ? "" : config.getPk());
	}
	...
	*/
	...
	@Override
	 public boolean configuredUsingAnnotations() {
	        return (config == null ? false : (config.configuredUsingAnnotations() ));
	}

	@Override
	public boolean configuredForMaterialization() {
		return (config == null ? false : (config.getStagingCacheName() != null));
	}
	
	private InfinispanManagedConnectionFactory getConfig() throws TranslatorException {
		if (config != null) {
			return this.config;
		}
		
		this.connectionDead = true;
		throw new RuntimeException(InfinispanManagedConnectionFactory.UTIL.getString("TEIID25012") );
	}
