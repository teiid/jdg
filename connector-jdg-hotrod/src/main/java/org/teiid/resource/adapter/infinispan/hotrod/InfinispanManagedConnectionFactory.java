...
public class InfinispanManagedConnectionFactory extends BasicManagedConnectionFactory {
	...
	private void validation() throws ResourceException {
		...
		if (authSASLMechanism != null) {
			...
			// optional, but both must be specified if they are used
			if ( (authPassword != null && authUserName == null) || (authPassword == null && authUserName != null)) {
		...
		try {
			if (usingAnnotations) {
				cacheSchemaConfigurator = (InfinispanSchemaDefinition) ReflectionHelper
						.create("org.teiid.resource.adapter.infinispan.hotrod.schema.AnnotationSchema", null, this.getClass().getClassLoader());
			} else {
				cacheSchemaConfigurator = (InfinispanSchemaDefinition) ReflectionHelper
						.create("org.teiid.resource.adapter.infinispan.hotrod.schema.ProtobufSchema", null, this.getClass().getClassLoader());
			}
